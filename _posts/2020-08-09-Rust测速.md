...
```
值得一提的是怎么gen数据，即怎么从一个很大的集合中随机选出一个元素。我使用了pbds，`*t.find_by_order(rand()%t.size())`{:c}。不知道有没有什么更简单的方法。(**2025-10-19 UPD：**这其实非常简单，维护一个key到下标的哈希表和一个下标到(key,value) pair的数组即可。)
